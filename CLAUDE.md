...
- **优先级判断**：当前最大缺口是消费端（🚧 批量提取分析）。消费端手动流程跑顺前，采集侧新能力默认冻结。2026-08-22 经项目拷问确认：冻结政策维持——消费端闭环（bundle 导出 → Claude Code 会话分析 → `analysis/<主题>/` 落盘）跑通前不新增采集侧能力；落盘规范以 [README.md](README.md)「分析产物规范」条目（`analysis/<主题>/`）为准。**2026-08-26 有条件解冻（一项）**：消费端闭环首例已落盘（shilanwei），且无字幕视频正是消费端分析的盲区——仅放行「no-subtitle 兜底 ASR 转写入库」（`asr backfill` → `asr-zh` 轨，产物直接进 bundle/export），其余采集侧维持冻结。
...
- **可复用功能必须代码化**（2026-08-24 经用户确认）：可复用的操作（产品功能、批量编排、常用查询）默认通过已沉淀的代码执行——CLI 命令或 `scripts/` 工具；代码缺能力就加代码（含测试与 skill 文档同步），禁止用会话内易失编排（内联脚本、手工挑选、绕过 CLI 直改数据）凑合。要绕过，先向用户确认；用户在场明确指示的临时操作视为已授权。**经确认的绕过发生后，必须把该操作登记为 CLI 完整度缺口**（落 [docs/plans/cli-completeness.md](docs/plans/cli-completeness.md) 的反向登记段；2026-08-25 经用户确认）——反向登记清零之日，即 CLI 完整之日。只读验证类查询豁免。探索性数据分析的最佳形态待定（形成惯例前先与用户对齐形式）。
