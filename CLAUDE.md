...
- **可复用功能必须代码化**（2026-08-24 经用户确认）：可复用的操作（产品功能、批量编排、常用查询）默认通过已沉淀的代码执行——CLI 命令或 `scripts/` 工具；代码缺能力就加代码（含测试与 skill 文档同步），禁止用会话内易失编排（内联脚本、手工挑选、绕过 CLI 直改数据）凑合。要绕过，先向用户确认；用户在场明确指示的临时操作视为已授权。只读验证类查询豁免。探索性数据分析的最佳形态待定（形成惯例前先与用户对齐形式）。
