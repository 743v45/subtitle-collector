...
// 列表项富化：用 json_extract 从 extra 取 tid/tname/tags，前端列表直接展示分区/标签，避免逐条再请求。
// tags 在 extra 是对象数组 [{tag_id,tag_name}]，这里降维成 tag_name 字符串数组；非合法 JSON → 空数组（不 500）。
function enrichItems(
  db: Database.Database,
  items: VideoListItemAdvanced[],
): Array<VideoListItemAdvanced & { tid: number | null; tname: string | null; tags: string[] }> {
  ...
  ).all(...ids) as Array<{ id: number; tid: number | null; tname: string | null; tags: string | null }>;
  ...
  return items.map((it) => {
    ...
    return { ...it, tid: r?.tid ?? null, tname: r?.tname ?? null, tags };
  });
}
