...
| 2 | 点开视频字幕按钮（中文字幕） | 服务端控制台看到 ingest-ack；popup「已收集」区块刷新（INGEST_RESULT 命中当前 bvid 触发本地+server 重查） |
...
| 11 | 打开**已采集过**的视频页，点扩展 popup | "已收集"区块以**本地**提取的数据显示：字幕轨数（可展开选轨复制）、分区、播放/点赞/投币/收藏/转发/弹幕数、标签；副标题显示"上次上报 ..."（来自 server） |
| 12 | 打开**未采集过**的视频页（视频本身有字幕），点 popup | "已收集"正常显示本地轨道/统计/标签（数据源是页面实时提取，与是否上报无关）；副标题显示"未上报到服务端" |
| 13 | 关掉服务端后点 popup | "已收集"区块**仍正常显示本地数据**（不依赖 server）；副标题显示"服务端未运行（一致性校验不可用）"；无 ⚠ 一致性标记 |
| 14 | 已采集视频再访问（仅 stat 数字变化） | popup 显示的 stat 是页面实时值；服务端 extra.stat 在下次 ingest 时更新为最新值，但 change_log 不增加（stat 波动不记变更） |
| 15 | 有字幕的视频页，展开「复制字幕」→ 选一轨 → 选格式（纯文本/带时间戳/SRT）→ 复制 | 剪贴板得到对应格式字幕；格式选择记忆，下次打开仍为上次选择 |
| 16 | 打开**无字幕**的视频（player API subtitles 数组空），点 popup | "已收集"显示"当前视频没有字幕"（区别于"有字幕但没获取到"的"正在获取当前视频信息..."） |
| 17 | 本地轨道数 ≠ server 轨数（如本地 3 轨、server 2 轨） | "已收集"标题旁出现 ⚠「字幕轨数不一致」badge，悬停看本地/服务端数值 |
| 18 | popup 上报开关 | 标签为「自动上报」（原「上报」）；手动补采按钮不变 |
...
| R2 | _待填_ | _待填_ | 本地数据源切换 + 字幕复制（折叠/选轨/格式记忆）+ 无字幕区分 + 轨数一致性校验 + 「上报」→「自动上报」改名 |
