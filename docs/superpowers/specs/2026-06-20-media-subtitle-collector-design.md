...
学 opencli 的 `Command`/`Result` 信封 + 握手 + 探活 + 重连（详见 `.claude/references/opencli-通信设计-映射.md`）：

- **Command**（服务端 → 扩展）：`{ id, action, ... }`。MVP action 至少含：
  - `ingest-ack`（确认上报，也可由扩展主动推 ingest 消息）
...
- **Result**（扩展 → 服务端）：`{ id, ok, data/error, errorCode? }`
- **扩展 → 服务端的主动消息**：`hello`（握手，带扩展版本）、`log`（日志转发）、`ingest`（被动采集的数据上报）
- **连接管理**：扩展 WS 客户端连 `ws://localhost:21527`；连前先 `GET /ping` 探活（避免 `new WebSocket()` 失败打 ERR 噪声，学 opencli）；指数退避重连 + keepalive alarm 兜底
- **安全**：loopback only + Origin 校验（只接 `chrome-extension://`）+ WS upgrade 时 verifyClient 拒非扩展 origin（学 opencli 防 WS CSRF）；不上 opencli 的 X-自定义头那道（loopback + 固定场景够）
...
**回退到 CDP 的触发条件**（留作判断锚点，不在 MVP 实现）：若实测发现 B 站字幕按钮必须真实鼠标事件（pointerdown 等）才响应、content script 的 `element.click()` 点不动，才在该步引入 CDP。当前判定 B 站字幕按钮为常规控件，②+3 足够。
...
**扩展 → 服务端（主动消息）：**
- `hello`：握手，`{ type:'hello', ext_version }`
- `log`：日志转发，`{ type:'log', level, msg, ts }`
- `ingest`：被动/主动采集的数据上报，payload 结构同下"ingest 数据结构"。服务端处理后回 `{ type:'ingest-ack', ok, inserted_tracks, skipped_tracks }`
- `result`：对服务端下发 Command 的执行结果，`{ id, ok, data/error }`

**服务端 → 扩展（Command）：**
- `{ id, action:'navigate', url }`：打开视频页
- `{ id, action:'operate', tab_hint, op }`：页面操作（如点字幕开关），content script 执行
- `{ id, action:'fetch-subtitle', bvid }`：触发某视频字幕抓取
- 其余 action（批量/定时/UP主/AI）：协议预留
...
**安全：** loopback only；WS upgrade `verifyClient` 拒非 `chrome-extension://` origin（防 WS CSRF，学 opencli）。

### 6.3 网页消费（HTTP，查询路径，只读，无鉴权）
...
- 连上 → 发 `hello`（扩展版本）
- 指数退避重连 + keepalive alarm 兜底（学 opencli）
