...
| `asr backfill` | server HTTP + B 站 + 本机 fireredasr | 无字幕视频兜底转写(no-subtitle 圈定 B 站):`--size <n>`(默认 5,先小样本实测速度,RTF≈0.2)/ `--page` / `--max-duration <秒>` / `--dry-run`(只圈定)/ `--cookie-file`(必配——nav 取 wbi keys 即需登录态,匿名 -101,或 $COLLECTOR_BILI_COOKIE_FILE)/ `--asr-url`(默认 127.0.0.1:5079);圈定→playurl 拉音轨→FireRedASR 转写→写回 asr-zh 轨,成功自动摘标(重跑跳过已完成);失败分类计数不中断批次 |
